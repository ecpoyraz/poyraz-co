...
/**
 * Pinned, scroll-scrubbed hero. "You believed" starts solid while the rest
 * of the sentence sits at low opacity. The remaining words, the MRR card,
 * and the counter step forward with the scrollbar — including the number
 * itself, which counts up (and back down) as you scroll.
 */
export function HeroMrr() {
  ...
  useGSAP(
    () => {
      ...
      // The low-opacity words turn solid one by one as scrolling begins.
      tl.to(
        ".hm-word-1",
    ...
    <div ref={root}>
      <div className="hm-scene relative flex h-[calc(100svh-6.5rem)] flex-col items-center justify-center overflow-hidden px-5">
        ...
        <h1 className="hero-drift-slow max-w-5xl text-center font-display text-4xl font-normal leading-[1.15] tracking-[-0.02em] sm:text-5xl md:text-6xl lg:text-7xl">
          <span className="text-foreground">You believed</span>{" "}
          <span className="hm-word hm-word-1 text-foreground">
            great product
          </span>
          <br />
          <span className="hm-word hm-word-2 text-foreground">
            would earn you
          </span>{" "}
